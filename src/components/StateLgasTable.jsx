...
import { StateLgaForm } from '../components'
...
const StateLgasTable = () => {
    const { state, deleteStateLga, getStateLgas } = useContext(GlobalContext)
    ...
    },
    {
        ...
        dataIndex: 'stateName',
        key: 'stateName',
        filters: state.countryStates[3].map(state => (
            {
                text: state.name,
                value: state.name
            }
        ...
        render: (record) => (
            <Space size='middle'>
                <Popover
                    ...
                    content={ <StateLgaForm item={details} />}
    ...
    return (
        <Fragment>
            <div className="tools-container" style={{ display: 'flex', flexDirection: 'row' }}>
                { (state.loggedIn && state.user.role !== "guest") && (
                    <div className="add-user" style={{ marginRight: '2pt'}}>
                        <Popover
                            ...
                            content={ <StateLgaForm item={defaultItem} />}
                ...
                { (state.loggedIn && state.user.role !== "guest") && (
                    <div className="button-container" style={{ marginRight: '2pt'}}>
                        <Button className='get-button' onClick={ () => getStateLgas() }>
                ...
                { (state.loggedIn && state.user.role !== "guest") && (
                    <div className="button-container">
                        <Button className='get-button' onClick={ () => exportToExcel(state.stateLgas[3], 'State_Lgas') }>
            ...
            <div className="table-container">
                <Table rowKey={ (record) => record.$key } scroll={scroll} className='table' columns={columns} dataSource={state.stateLgas[3]} />
            </div>
            <div className="footer">
                { state.stateLgas[3].length > 0 && (
                    <Text className='footer-text' level={1}>
                        <b>Currently there are { state.stateLgas[3].length } Items !!!</b>
                    </Text>
                )}
                { state.stateLgas[3].length < 1 && (
                    <Text className='footer-text' level={1}>
