...
import { cbtCategories, cbtClasses, terms, utmeSubjects } from '../services/userHelper'

const { Option } = Select
const { Item } = Form
const { TextArea } = Input

const UtmeAddQuestionForm = ({ question }) => {
    const [form] = Form.useForm()
    const [formMessage, setFormMessage] = useState('')
    const [formError, setFormError] = useState('')
    const { addQuestion, editQuestion, state, production } = useContext(GlobalContext)
        ...
        <div className="form-group">
            ...
            <Form name="basic" form={ form } validateMessages={ validateMessages } initialValues={{ remember: true }} onFinish={ onFinish } onFinishFailed={ onFinishFailed } autoComplete="off">
                <div className="form-controller">
                    <Item className='form-item-text-area' label="Question" name="question" rules={[ { required: true } ]} hasFeedback>
                        <TextArea prefix={<SolutionOutlined />} placeholder="Input Question" allowClear />
                    </Item>
                </div>
                <div className="form-controller">
                ...
                <div className="button-controller">
                    <Item>
                        <Button className="submit-button" type="primary" htmlType="submit">
                            { question.question === "" ? "Submit" : "Update" }
                        </Button>
                    </Item>
                    { question.question === "" && (
                        <Item>
                            <Button className="reset-button" type="primary" danger onClick={ onReset }>
                                Reset
                            </Button>
                        </Item>
                    )}
...
export default UtmeAddQuestionForm
